...
def saveSchedule(link, hour, minutes, durH, durMin, day):
    ...
    if areIntegers and validity:
        try:
            ...
            f = open("data_files/schedules.txt", "a")
            f.write(info)
            f.write('\n')
            f.close()

            f = open("data_files/prettySchedules.txt", "r")
...
def getRecs():
    ...
    """
    f = open("data_files/schedules.txt", "r")
    toReturn = f.read()
    if(toReturn == ""):
        toReturn = "No Scheduled Recordings yet"
    f.close()
...
def deleteSchedule(num):
    ...
    if num.isdigit():
        ...
        return
    f1 = open("data_files/schedules.txt", "r")
    f2 = open("data_files/prettySchedules.txt", "r")
    ...
    if 1<= num <=numOfLines:
        ...
        f1 = open("data_files/schedules.txt", "w")
        f2 = open("data_files/prettySchedules.txt", "w")
...
def recordingDone(link):
    f = open("data_files/recCompleted.txt", "r")
    fileLink = f.read()
    f.close()
    if link == fileLink:
        return True
